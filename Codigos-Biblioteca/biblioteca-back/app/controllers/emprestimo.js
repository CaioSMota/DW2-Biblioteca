...
const { getEmprestimo, emprestimoPost, getProcuraEmprestimo } = require ('../models/emprestimo');
...
module.exports.listarProcuraEmprestimo = function (app,req,res){
	let connection = dbConnection();
	getProcuraEmprestimo(req, connection, function(error, result) {
			if(error){
			res.send(error);
		}else{
			res.send(result);
